import { classesToTokens, createElement, elementChildren, elementIndex, elementOffset, elementOuterSize, elementParents, elementTransitionEnd, getDocument, getRotateFix, getSlideTransformEl, getTranslate, getWindow, isObject, makeElementsArray, nextTick, now, setCSSProperty, setInnerHTML } from "./utils-CQTGcahf.js";
...
function Virtual({ swiper, extendParams, on, emit }) {
	...
	const tempDOM = document$1.createElement("div");
...
function Keyboard({ swiper, extendParams, on, emit }) {
	...
	function handle(event$1) {
		...
		if (document$1.activeElement && (document$1.activeElement.isContentEditable || document$1.activeElement.nodeName && (document$1.activeElement.nodeName.toLowerCase() === "input" || document$1.activeElement.nodeName.toLowerCase() === "textarea"))) return;
	...
	function enable() {
		...
		document$1.addEventListener("keydown", handle);
		swiper.keyboard.enabled = true;
	}
	function disable() {
		if (!swiper.keyboard.enabled) return;
		document$1.removeEventListener("keydown", handle);
		swiper.keyboard.enabled = false;
	}
	on("init", () => {
...
function Mousewheel({ swiper, extendParams, on, emit }) {
	...
	function animateSlider(newEvent) {
		...
		lastScrollTime = new window$1.Date().getTime();
		return false;
	}
	...
	function handle(event$1) {
		...
		if (!swiper.enabled) return;
		if (event$1.target.closest(`.${swiper.params.mousewheel.noMousewheelClass}`)) return;
		const params = swiper.params.mousewheel;
...
function Scrollbar({ swiper, extendParams, on, emit }) {
	...
	function events(method) {
		...
		document$1[eventMethod]("pointermove", onDragMove, activeListener);
		document$1[eventMethod]("pointerup", onDragEnd, passiveListener);
	}
	...
	function init() {
		...
		if (!el && typeof params.el === "string") {
			el = document$1.querySelectorAll(params.el);
			if (!el.length) return;
...
function Zoom({ swiper, extendParams, on, emit }) {
	...
	function getScaleOrigin() {
		...
		return [(evCache[0].pageX + (evCache[1].pageX - evCache[0].pageX) / 2 - box.x - window$1.scrollX) / currentScale, (evCache[0].pageY + (evCache[1].pageY - evCache[0].pageY) / 2 - box.y - window$1.scrollY) / currentScale];
	}
	...
	function onTouchStart(e) {
		...
		const event$1 = evCache.length > 0 ? evCache[0] : e;
		image.touchesStart.x = event$1.pageX;
		image.touchesStart.y = event$1.pageY;
	}
	...
	function onMouseMove(e) {
		...
		if (!eventWithinSlide(e) || !eventWithinZoomContainer(e)) return;
		const currentTransform = window$1.getComputedStyle(gesture.imageWrapEl).transform;
		const matrix = new window$1.DOMMatrix(currentTransform);
	...
	function zoomIn(e) {
		...
		if (e && !(currentScale === 1 && forceZoomRatio)) {
			...
			offsetX = elementOffset(gesture.slideEl).left + window$1.scrollX;
			offsetY = elementOffset(gesture.slideEl).top + window$1.scrollY;
...
function History({ swiper, extendParams, on }) {
	...
	const getPathValues = (urlOverride) => {
		...
		else location = window$1.location;
	...
	const setHistory = (key, index) => {
		...
		if (swiper.params.history.replaceState) window$1.history.replaceState({ value }, null, value);
		else window$1.history.pushState({ value }, null, value);
	};
	...
	const init = () => {
		...
		if (!paths.key && !paths.value) {
			if (!swiper.params.history.replaceState) window$1.addEventListener("popstate", setHistoryPopState);
			return;
		}
	...
	const destroy = () => {
		const window$1 = getWindow();
		if (!swiper.params.history.replaceState) window$1.removeEventListener("popstate", setHistoryPopState);
	};
	on("init", () => {
...
function HashNavigation({ swiper, extendParams, emit, on }) {
	...
	const setHash = () => {
		...
		if (swiper.params.hashNavigation.replaceState && window$1.history && window$1.history.replaceState) {
			window$1.history.replaceState(null, null, `#${activeSlideHash}` || "");
			emit("hashSet");
		} else {
			document$1.location.hash = activeSlideHash || "";
			emit("hashSet");
		}
	};
	const init = () => {
		...
		if (swiper.params.hashNavigation.watchState) window$1.addEventListener("hashchange", onHashChange);
	};
	const destroy = () => {
		if (swiper.params.hashNavigation.watchState) window$1.removeEventListener("hashchange", onHashChange);
	};
	on("init", () => {
...
function Autoplay({ swiper, extendParams, on, emit, params }) {
	...
	const onVisibilityChange = () => {
		...
		if (document$1.visibilityState === "visible") resume();
	};
	...
	on("beforeInit", () => {
		...
		if (typeof thumbs.swiper === "string" || thumbs.swiper instanceof HTMLElement) {
			...
			const getThumbsElementAndInit = () => {
				const thumbsElement = typeof thumbs.swiper === "string" ? document$1.querySelector(thumbs.swiper) : thumbs.swiper;
