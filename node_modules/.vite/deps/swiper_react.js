...
import { require_react } from "./react-tD2ZY1nN.js";
import { animateCSSModeScroll, createElement, deleteProps, elementChildren, elementIndex, elementIsChildOf, elementNextAll, elementOuterSize, elementParents, elementPrevAll, elementStyle, extend as extend$1, getDocument, getTranslate, getWindow, nextTick, now, setCSSProperty, setInnerHTML, showWarning } from "./utils-CQTGcahf.js";
...
function calcSupport() {
	...
	return {
		smoothScroll: document$1.documentElement && document$1.documentElement.style && "scrollBehavior" in document$1.documentElement.style,
		touch: !!("ontouchstart" in window$1 || window$1.DocumentTouch && document$1 instanceof window$1.DocumentTouch)
	};
}
function getSupport() {
...
function calcDevice({ userAgent } = {}) {
	const support$1 = getSupport();
	const window$1 = getWindow();
	const platform = window$1.navigator.platform;
	const ua = userAgent || window$1.navigator.userAgent;
	const device = {
		ios: false,
		android: false
	};
	const screenWidth = window$1.screen.width;
	const screenHeight = window$1.screen.height;
...
function calcBrowser() {
	...
	if (isSafari()) {
		const ua = String(window$1.navigator.userAgent);
		if (ua.includes("Version/")) {
			const [major, minor] = ua.split("Version/")[1].split(" ")[0].split(".").map((num) => Number(num));
			needPerspectiveFix = major < 16 || major === 16 && minor < 2;
		}
	}
	const isWebView = /(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/i.test(window$1.navigator.userAgent);
...
function Resize({ swiper, on, emit }) {
	...
	const createObserver = () => {
		...
		observer = new ResizeObserver((entries) => {
			animationFrame = window$1.requestAnimationFrame(() => {
	...
	const removeObserver = () => {
		if (animationFrame) window$1.cancelAnimationFrame(animationFrame);
	...
	on("init", () => {
		if (swiper.params.resizeObserver && typeof window$1.ResizeObserver !== "undefined") {
			createObserver();
			return;
		}
		window$1.addEventListener("resize", resizeHandler);
		window$1.addEventListener("orientationchange", orientationChangeHandler);
	});
	on("destroy", () => {
		removeObserver();
		window$1.removeEventListener("resize", resizeHandler);
		window$1.removeEventListener("orientationchange", orientationChangeHandler);
	});
}
function Observer({ swiper, extendParams, on, emit }) {
	...
	const attach = (target, options = {}) => {
		const observer = new (window$1.MutationObserver || window$1.WebkitMutationObserver)((mutations) => {
			if (swiper.__preventObserver__) return;
			if (mutations.length === 1) {
				emit("observerUpdate", mutations[0]);
				return;
			}
			const observerUpdate = function observerUpdate$1() {
				emit("observerUpdate", mutations[0]);
			};
			if (window$1.requestAnimationFrame) window$1.requestAnimationFrame(observerUpdate);
			else window$1.setTimeout(observerUpdate, 0);
		});
...
var eventsEmitter = {
	on(events$2, handler, priority) {
		const self = this;
		if (!self.eventsListeners || self.destroyed) return self;
		if (typeof handler !== "function") return self;
		const method = priority ? "unshift" : "push";
		events$2.split(" ").forEach((event) => {
			if (!self.eventsListeners[event]) self.eventsListeners[event] = [];
			self.eventsListeners[event][method](handler);
		});
		return self;
	},
	once(events$2, handler, priority) {
		const self = this;
		if (!self.eventsListeners || self.destroyed) return self;
		if (typeof handler !== "function") return self;
		function onceHandler(...args) {
			self.off(events$2, onceHandler);
			if (onceHandler.__emitterProxy) delete onceHandler.__emitterProxy;
			handler.apply(self, args);
		}
		onceHandler.__emitterProxy = handler;
		return self.on(events$2, onceHandler, priority);
	},
	...
	emit(...args) {
		...
		if (typeof args[0] === "string" || Array.isArray(args[0])) {
			...
			events$2 = args[0].events;
			data = args[0].data;
			context = args[0].context || self;
		}
		data.unshift(context);
		(Array.isArray(events$2) ? events$2 : events$2.split(" ")).forEach((event) => {
...
function updateSlides() {
	...
	for (let i = 0; i < slidesLength; i += 1) {
		slideSize = 0;
		const slide$1 = slides[i];
		if (slide$1) {
			if (gridEnabled) swiper.grid.updateSlide(i, slide$1, slides);
			if (elementStyle(slide$1, "display") === "none") continue;
		}
		if (isVirtual && params.slidesPerView === "auto") {
			if (params.virtual.slidesPerViewAutoSlideSize) slideSize = params.virtual.slidesPerViewAutoSlideSize;
			if (slideSize && slide$1) {
				if (params.roundLengths) slideSize = Math.floor(slideSize);
				slide$1.style[swiper.getDirectionLabel("width")] = `${slideSize}px`;
			}
		} else if (params.slidesPerView === "auto") {
			if (shouldResetSlideSize) slide$1.style[swiper.getDirectionLabel("width")] = ``;
			const slideStyles = getComputedStyle(slide$1);
			const currentTransform = slide$1.style.transform;
			const currentWebKitTransform = slide$1.style.webkitTransform;
			if (currentTransform) slide$1.style.transform = "none";
			if (currentWebKitTransform) slide$1.style.webkitTransform = "none";
			if (params.roundLengths) slideSize = swiper.isHorizontal() ? elementOuterSize(slide$1, "width", true) : elementOuterSize(slide$1, "height", true);
			else {
				...
				if (boxSizing && boxSizing === "border-box") slideSize = width + marginLeft + marginRight;
					...
					const { clientWidth, offsetWidth } = slide$1;
					slideSize = width + paddingLeft + paddingRight + marginLeft + marginRight + (offsetWidth - clientWidth);
				}
			}
			if (currentTransform) slide$1.style.transform = currentTransform;
			if (currentWebKitTransform) slide$1.style.webkitTransform = currentWebKitTransform;
			if (params.roundLengths) slideSize = Math.floor(slideSize);
		} else {
			slideSize = (swiperSize - (params.slidesPerView - 1) * spaceBetween) / params.slidesPerView;
			if (params.roundLengths) slideSize = Math.floor(slideSize);
			if (slide$1) slide$1.style[swiper.getDirectionLabel("width")] = `${slideSize}px`;
		}
		if (slide$1) slide$1.swiperSlideSize = slideSize;
...
function updateAutoHeight(speed) {
	...
	if (swiper.params.slidesPerView !== "auto" && swiper.params.slidesPerView > 1) if (swiper.params.centeredSlides) (swiper.visibleSlides || []).forEach((slide$1) => {
		activeSlides.push(slide$1);
	});
...
function updateSlidesProgress(translate$1 = this && this.translate || 0) {
	const swiper = this;
	const params = swiper.params;
	const { slides, rtlTranslate: rtl, snapGrid } = swiper;
	if (slides.length === 0) return;
	if (typeof slides[0].swiperSlideOffset === "undefined") swiper.updateSlidesOffset();
	let offsetCenter = -translate$1;
	if (rtl) offsetCenter = translate$1;
	...
	for (let i = 0; i < slides.length; i += 1) {
		const slide$1 = slides[i];
		let slideOffset = slide$1.swiperSlideOffset;
		if (params.cssMode && params.centeredSlides) slideOffset -= slides[0].swiperSlideOffset;
		const slideProgress = (offsetCenter + (params.centeredSlides ? swiper.minTranslate() : 0) - slideOffset) / (slide$1.swiperSlideSize + spaceBetween);
		const originalSlideProgress = (offsetCenter - snapGrid[0] + (params.centeredSlides ? swiper.minTranslate() : 0) - slideOffset) / (slide$1.swiperSlideSize + spaceBetween);
		const slideBefore = -(offsetCenter - slideOffset);
		const slideAfter = slideBefore + swiper.slidesSizesGrid[i];
		const isFullyVisible = slideBefore >= 0 && slideBefore <= swiper.size - swiper.slidesSizesGrid[i];
		const isVisible = slideBefore >= 0 && slideBefore < swiper.size - 1 || slideAfter > 1 && slideAfter <= swiper.size || slideBefore <= 0 && slideAfter >= swiper.size;
		if (isVisible) {
			swiper.visibleSlides.push(slide$1);
			swiper.visibleSlidesIndexes.push(i);
		}
		toggleSlideClasses$1(slide$1, isVisible, params.slideVisibleClass);
		toggleSlideClasses$1(slide$1, isFullyVisible, params.slideFullyVisibleClass);
		slide$1.progress = rtl ? -slideProgress : slideProgress;
		slide$1.originalProgress = rtl ? -originalSlideProgress : originalSlideProgress;
	}
}
function updateProgress(translate$1) {
	const swiper = this;
	if (typeof translate$1 === "undefined") {
		const multiplier = swiper.rtlTranslate ? -1 : 1;
		translate$1 = swiper && swiper.translate && swiper.translate * multiplier || 0;
	}
	const params = swiper.params;
	...
	if (translatesDiff === 0) {
		...
		progress = (translate$1 - swiper.minTranslate()) / translatesDiff;
		const isBeginningRounded = Math.abs(translate$1 - swiper.minTranslate()) < 1;
		const isEndRounded = Math.abs(translate$1 - swiper.maxTranslate()) < 1;
	...
	if (params.loop) {
		...
		const translateAbs = Math.abs(translate$1);
	...
	if (params.watchSlidesProgress || params.centeredSlides && params.autoHeight) swiper.updateSlidesProgress(translate$1);
...
function getActiveIndexByTranslate(swiper) {
	...
	const translate$1 = swiper.rtlTranslate ? swiper.translate : -swiper.translate;
	let activeIndex;
	for (let i = 0; i < slidesGrid.length; i += 1) if (typeof slidesGrid[i + 1] !== "undefined") {
		if (translate$1 >= slidesGrid[i] && translate$1 < slidesGrid[i + 1] - (slidesGrid[i + 1] - slidesGrid[i]) / 2) activeIndex = i;
		else if (translate$1 >= slidesGrid[i] && translate$1 < slidesGrid[i + 1]) activeIndex = i + 1;
	} else if (translate$1 >= slidesGrid[i]) activeIndex = i;
...
function updateActiveIndex(newActiveIndex) {
	...
	const getVirtualRealIndex = (aIndex) => {
		let realIndex$1 = aIndex - swiper.virtual.slidesBefore;
		if (realIndex$1 < 0) realIndex$1 = swiper.virtual.slides.length + realIndex$1;
		if (realIndex$1 >= swiper.virtual.slides.length) realIndex$1 -= swiper.virtual.slides.length;
		return realIndex$1;
	};
	if (typeof activeIndex === "undefined") activeIndex = getActiveIndexByTranslate(swiper);
	if (snapGrid.indexOf(translate$1) >= 0) snapIndex = snapGrid.indexOf(translate$1);
	else {
...
function updateClickedSlide(el, path) {
	const swiper = this;
	const params = swiper.params;
	let slide$1 = el.closest(`.${params.slideClass}, swiper-slide`);
	if (!slide$1 && swiper.isElement && path && path.length > 1 && path.includes(el)) [...path.slice(path.indexOf(el) + 1, path.length)].forEach((pathEl) => {
		if (!slide$1 && pathEl.matches && pathEl.matches(`.${params.slideClass}, swiper-slide`)) slide$1 = pathEl;
	});
	let slideFound = false;
	let slideIndex;
	if (slide$1) {
		for (let i = 0; i < swiper.slides.length; i += 1) if (swiper.slides[i] === slide$1) {
			slideFound = true;
			slideIndex = i;
			break;
		}
	}
	if (slide$1 && slideFound) {
		swiper.clickedSlide = slide$1;
		if (swiper.virtual && swiper.params.virtual.enabled) swiper.clickedIndex = parseInt(slide$1.getAttribute("data-swiper-slide-index"), 10);
...
function getSwiperTranslate(axis = this.isHorizontal() ? "x" : "y") {
	const swiper = this;
	const { params, rtlTranslate: rtl, translate: translate$1, wrapperEl } = swiper;
	if (params.virtualTranslate) return rtl ? -translate$1 : translate$1;
	if (params.cssMode) return translate$1;
	let currentTranslate = getTranslate(wrapperEl, axis);
	currentTranslate += swiper.cssOverflowAdjustment();
	if (rtl) currentTranslate = -currentTranslate;
	return currentTranslate || 0;
}
function setTranslate(translate$1, byController) {
	const swiper = this;
	const { rtlTranslate: rtl, params, wrapperEl, progress } = swiper;
	let x = 0;
	let y = 0;
	const z = 0;
	if (swiper.isHorizontal()) x = rtl ? -translate$1 : translate$1;
	else y = translate$1;
	...
	else newProgress = (translate$1 - swiper.minTranslate()) / translatesDiff;
	if (newProgress !== progress) swiper.updateProgress(translate$1);
...
function translateTo(translate$1 = 0, speed = this.params.speed, runCallbacks = true, translateBounds = true, internal) {
	const swiper = this;
	const { params, wrapperEl } = swiper;
	if (swiper.animating && params.preventInteractionOnTransition) return false;
	const minTranslate$1 = swiper.minTranslate();
	const maxTranslate$1 = swiper.maxTranslate();
	let newTranslate;
	if (translateBounds && translate$1 > minTranslate$1) newTranslate = minTranslate$1;
	else if (translateBounds && translate$1 < maxTranslate$1) newTranslate = maxTranslate$1;
	else newTranslate = translate$1;
	...
	if (speed === 0) {
		...
		if (!swiper.animating) {
			...
			if (!swiper.onTranslateToWrapperTransitionEnd) swiper.onTranslateToWrapperTransitionEnd = function transitionEnd$1(e) {
...
function slideTo(index = 0, speed, runCallbacks = true, internal, initial) {
	...
	if (snapIndex >= snapGrid.length) snapIndex = snapGrid.length - 1;
	const translate$1 = -snapGrid[snapIndex];
	if (params.normalizeSlideIndex) for (let i = 0; i < slidesGrid.length; i += 1) {
		const normalizedTranslate = -Math.floor(translate$1 * 100);
	...
	if (swiper.initialized && slideIndex !== activeIndex) {
		if (!swiper.allowSlideNext && (rtl ? translate$1 > swiper.translate && translate$1 > swiper.minTranslate() : translate$1 < swiper.translate && translate$1 < swiper.minTranslate())) return false;
		if (!swiper.allowSlidePrev && translate$1 > swiper.translate && translate$1 > swiper.maxTranslate()) {
			if ((activeIndex || 0) !== slideIndex) return false;
		}
	}
	if (slideIndex !== (previousIndex || 0) && runCallbacks) swiper.emit("beforeSlideChangeStart");
	swiper.updateProgress(translate$1);
	let direction;
	if (slideIndex > activeIndex) direction = "next";
	else if (slideIndex < activeIndex) direction = "prev";
	else direction = "reset";
	const isVirtual = swiper.virtual && swiper.params.virtual.enabled;
	if (!(isVirtual && initial) && (rtl && -translate$1 === swiper.translate || !rtl && translate$1 === swiper.translate)) {
		swiper.updateActiveIndex(slideIndex);
		if (params.autoHeight) swiper.updateAutoHeight();
		swiper.updateSlidesClasses();
		if (params.effect !== "slide") swiper.setTranslate(translate$1);
		if (direction !== "reset") {
	...
	if (params.cssMode) {
		const isH = swiper.isHorizontal();
		const t = rtl ? translate$1 : -translate$1;
		if (speed === 0) {
			if (isVirtual) {
...
function slidePrev(speed, runCallbacks = true, internal) {
	...
	const translate$1 = rtlTranslate ? swiper.translate : -swiper.translate;
	function normalize(val) {
		if (val < 0) return -Math.floor(Math.abs(val));
		return Math.floor(val);
	}
	const normalizedTranslate = normalize(translate$1);
...
function slideToClosest(speed, runCallbacks = true, internal, threshold = .5) {
	...
	const translate$1 = swiper.rtlTranslate ? swiper.translate : -swiper.translate;
	if (translate$1 >= swiper.snapGrid[snapIndex]) {
		const currentSnap = swiper.snapGrid[snapIndex];
		const nextSnap = swiper.snapGrid[snapIndex + 1];
		if (translate$1 - currentSnap > (nextSnap - currentSnap) * threshold) index += swiper.params.slidesPerGroup;
	} else {
		const prevSnap = swiper.snapGrid[snapIndex - 1];
		const currentSnap = swiper.snapGrid[snapIndex];
		if (translate$1 - prevSnap <= (currentSnap - prevSnap) * threshold) index -= swiper.params.slidesPerGroup;
	}
...
function loopFix({ slideRealIndex, slideTo: slideTo$1 = true, direction, setTranslate: setTranslate$1, activeSlideIndex, initial, byController, byMousewheel } = {}) {
	const swiper = this;
	...
	const activeColIndexWithShift = (gridEnabled ? slides[activeSlideIndex].column : activeSlideIndex) + (bothDirections && typeof setTranslate$1 === "undefined" ? -slidesPerView / 2 + .5 : 0);
	if (activeColIndexWithShift < loopedSlides) {
		...
		for (let i = 0; i < loopedSlides - activeColIndexWithShift; i += 1) {
			...
			if (gridEnabled) {
				...
				for (let i$1 = slides.length - 1; i$1 >= 0; i$1 -= 1) if (slides[i$1].column === colIndexToPrepend) prependSlidesIndexes.push(i$1);
		...
		for (let i = 0; i < slidesAppended; i += 1) {
			const index = i - Math.floor(i / cols) * cols;
			if (gridEnabled) slides.forEach((slide$1, slideIndex) => {
				if (slide$1.column === index) appendSlidesIndexes.push(slideIndex);
			});
	...
	else if (gridEnabled && (prependSlidesIndexes.length > 0 && isPrev || appendSlidesIndexes.length > 0 && isNext)) swiper.slides.forEach((slide$1, slideIndex) => {
		swiper.grid.updateSlide(slideIndex, slide$1, swiper.slides);
	});
	if (params.watchSlidesProgress) swiper.updateSlidesOffset();
	if (slideTo$1) {
	...
	if (swiper.controller && swiper.controller.control && !byController) {
		...
		if (Array.isArray(swiper.controller.control)) swiper.controller.control.forEach((c) => {
			if (!c.destroyed && c.params.loop) c.loopFix({
				...loopParams,
				slideTo: c.params.slidesPerView === params.slidesPerView ? slideTo$1 : false
			});
		});
		else if (swiper.controller.control instanceof swiper.constructor && swiper.controller.control.params.loop) swiper.controller.control.loopFix({
			...loopParams,
			slideTo: swiper.controller.control.params.slidesPerView === params.slidesPerView ? slideTo$1 : false
		});
	}
	swiper.emit("loopFix");
}
...
function preventEdgeSwipe(swiper, event, startX) {
	...
	if (edgeSwipeDetection && (startX <= edgeSwipeThreshold || startX >= window$1.innerWidth - edgeSwipeThreshold)) {
...
function onTouchStart(event) {
	...
	if (document$1.activeElement && document$1.activeElement.matches(data.focusableElements) && document$1.activeElement !== targetEl && (e.pointerType === "mouse" || e.pointerType !== "mouse" && !targetEl.matches(data.focusableElements))) document$1.activeElement.blur();
...
function onTouchMove(event) {
	...
	if (document$1.activeElement && document$1.activeElement.matches(data.focusableElements) && document$1.activeElement !== e.target && e.pointerType !== "mouse") document$1.activeElement.blur();
	if (document$1.activeElement) {
		if (e.target === document$1.activeElement && e.target.matches(data.focusableElements)) {
			data.isMoved = true;
...
function onTouchEnd(event) {
	...
	for (let i = 0; i < slidesGrid.length; i += i < params.slidesPerGroupSkip ? 1 : params.slidesPerGroup) {
		const increment$1 = i < params.slidesPerGroupSkip - 1 ? 1 : params.slidesPerGroup;
		if (typeof slidesGrid[i + increment$1] !== "undefined") {
			if (swipeToLast || currentPos >= slidesGrid[i] && currentPos < slidesGrid[i + increment$1]) {
				stopIndex = i;
...
var events = (swiper, method) => {
	...
	document$1[domMethod]("touchstart", swiper.onDocumentTouchStart, {
		passive: false,
		capture
	});
	el[domMethod]("touchstart", swiper.onTouchStart, { passive: false });
	el[domMethod]("pointerdown", swiper.onTouchStart, { passive: false });
	document$1[domMethod]("touchmove", swiper.onTouchMove, {
		passive: false,
		capture
	});
	document$1[domMethod]("pointermove", swiper.onTouchMove, {
		passive: false,
		capture
	});
	document$1[domMethod]("touchend", swiper.onTouchEnd, { passive: true });
	document$1[domMethod]("pointerup", swiper.onTouchEnd, { passive: true });
	document$1[domMethod]("pointercancel", swiper.onTouchEnd, { passive: true });
	document$1[domMethod]("touchcancel", swiper.onTouchEnd, { passive: true });
	document$1[domMethod]("pointerout", swiper.onTouchEnd, { passive: true });
	document$1[domMethod]("pointerleave", swiper.onTouchEnd, { passive: true });
	document$1[domMethod]("contextmenu", swiper.onTouchEnd, { passive: true });
...
function setBreakpoint() {
	...
	const breakpoints$1 = params.breakpoints;
	if (!breakpoints$1 || breakpoints$1 && Object.keys(breakpoints$1).length === 0) return;
	const document$1 = getDocument();
	const breakpointsBase = params.breakpointsBase === "window" || !params.breakpointsBase ? params.breakpointsBase : "container";
	const breakpointContainer = ["window", "container"].includes(params.breakpointsBase) || !params.breakpointsBase ? swiper.el : document$1.querySelector(params.breakpointsBase);
	const breakpoint = swiper.getBreakpoint(breakpoints$1, breakpointsBase, breakpointContainer);
	if (!breakpoint || swiper.currentBreakpoint === breakpoint) return;
	const breakpointParams = (breakpoint in breakpoints$1 ? breakpoints$1[breakpoint] : void 0) || swiper.originalParams;
...
function getBreakpoint(breakpoints$1, base = "window", containerEl) {
	if (!breakpoints$1 || base === "container" && !containerEl) return void 0;
	let breakpoint = false;
	const window$1 = getWindow();
	const currentHeight = base === "window" ? window$1.innerHeight : containerEl.clientHeight;
	const points = Object.keys(breakpoints$1).map((point) => {
	...
	for (let i = 0; i < points.length; i += 1) {
		...
		if (base === "window") {
			if (window$1.matchMedia(`(min-width: ${value}px)`).matches) breakpoint = point;
...
var Swiper$1 = class Swiper$1 {
	constructor(...args) {
		...
		const document$1 = getDocument();
		if (params.el && typeof params.el === "string" && document$1.querySelectorAll(params.el).length > 1) {
			const swipers = [];
			document$1.querySelectorAll(params.el).forEach((containerEl) => {
...
function uniqueClasses(classNames = "") {
	const classes$1 = classNames.split(" ").map((c) => c.trim()).filter((c) => !!c);
	const unique = [];
	classes$1.forEach((c) => {
...
function getParams(obj = {}, splitEvents = true) {
	...
	Object.keys(plainObj).forEach((key) => {
		...
		else if (key.search(/on[A-Z]/) === 0 && typeof obj[key] === "function") if (splitEvents) events$2[`${key[2].toLowerCase()}${key.substr(3)}`] = obj[key];
...
function processChildren(c) {
	...
	import_react.Children.toArray(c).forEach((child) => {
		...
		else if (child.props && child.props.children) processChildren(child.props.children).forEach((slide$1) => slides.push(slide$1));
	});
	return slides;
}
function getChildren(c) {
	...
	import_react.Children.toArray(c).forEach((child) => {
		if (isChildSwiperSlide(child)) slides.push(child);
			...
			if (foundSlides.length > 0) foundSlides.forEach((slide$1) => slides.push(slide$1));
...
var Swiper = /* @__PURE__ */ (0, import_react.forwardRef)(({ className, tag: Tag = "div", wrapperTag: WrapperTag = "div", children, onSwiper,...rest } = {}, externalElRef) => {
	...
	const { params: swiperParams, passedParams, rest: restProps, events: events$2 } = getParams(rest);
	const { slides, slots } = getChildren(children);
	const onBeforeBreakpoint = () => {
		setBreakpointChanged(!breakpointChanged);
	};
	Object.assign(swiperParams.on, { _containerClasses(swiper, classes$1) {
		setContainerClasses(classes$1);
	} });
	const initSwiper = () => {
		Object.assign(swiperParams.on, events$2);
		eventsAssigned = true;
	...
	const attachEvents$1 = () => {
		if (eventsAssigned || !events$2 || !swiperRef.current) return;
		Object.keys(events$2).forEach((eventName) => {
			swiperRef.current.on(eventName, events$2[eventName]);
		});
	};
	const detachEvents$1 = () => {
		if (!events$2 || !swiperRef.current) return;
		Object.keys(events$2).forEach((eventName) => {
			swiperRef.current.off(eventName, events$2[eventName]);
		});
	};
